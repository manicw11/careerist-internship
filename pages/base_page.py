...
class Page:
    ...
    def wait_to_disappear(self, locator):
        self.wait.until(EC.invisibility_of_element_located(locator), message=f'Element "{locator}" is still visible')

    def wait_for_url_change(self, initial_url):
        self.wait.until(EC.url_changes(initial_url), message=f'Url "{initial_url}" did not change')
